...
export default async function ExteriorPaintingPage() {
  const data = await fetchGraphQL<ServicePageData>(SERVICE_PAGE_QUERY, { id: 13 });
  const sp = data?.page?.servicePage;

  const heading = sp?.serviceHeading || "Exterior painting";
  const subtext = sp?.serviceSubtext || "Professional exterior painting for homes in the local area. Jason provides careful preparation, quality materials and weather-resistant finishes to keep the outside of your home looking its best.";
  const contentHeading = sp?.serviceContentHeading || "Protecting and refreshing your home's exterior";
  const contentText = sp?.serviceContentText || "From house walls and fascias to window frames and garden fences, Jason provides reliable exterior painting with proper preparation and durable, weatherproof finishes. Every job is completed neatly and professionally.";
  const heroImage = sp?.serviceHeroImage?.node?.sourceUrl || "https://images.unsplash.com/photo-1562259949-e8e7689d7828?w=800&q=80";
  const heroImageAlt = sp?.serviceHeroImage?.node?.altText || "Exterior of a freshly painted house";
  ...
  const features = sp?.serviceFeatures ? sp.serviceFeatures.split("\n").filter(Boolean) : ["House exterior walls", "Fascias & soffits", "Window frames & doors", "Garden walls & fences", "Weatherproof finishes", "Surface preparation"];
  const ctaHeading = sp?.serviceCtaHeading || "Need the outside of your home painted?";
  ...
  return (
    <>
      <JsonLd data={serviceSchema("Exterior Painting", "Professional exterior house painting for homes in Wellington, Taunton, Tiverton and surrounding areas. Fascias, soffits, window frames, doors and garden walls.", "https://jason-chapman.vercel.app/services/exterior-painting")} />
      {/* Hero */}
          ...
          <div className="grid md:grid-cols-3 gap-gutter">
            <div className="group relative bg-white rounded-2xl p-8 shadow-[0_1px_3px_rgba(0,0,0,0.04)] border border-slate-100 hover:shadow-[0_8px_30px_rgba(254,126,4,0.08)] hover:border-primary/20 transition-all duration-300">
              ...
              <p className="text-slate-500 leading-relaxed">
                ...
                properly before applying paint, so the result looks good and
